...
- Upload section: three file inputs (customers CSV, accounts CSV, transactions CSV) + Submit
  ...
  - Each POST returns `{ "jobId": "..." }` — collect these jobIds in React state
- Jobs table: jobId, type, status, total / processed / failed counts, submitted by, started / completed at
  - Table is rendered from in-memory state only — **there is no `GET /api/v1/jobs` list endpoint**
  - Poll each jobId individually: `GET /api/v1/jobs/{jobId}` every 5s while status is `RUNNING`
  - Stop polling when all jobs reach a terminal status (`COMPLETED` or `FAILED`)
